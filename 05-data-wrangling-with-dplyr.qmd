...
We will use the [comic-characters.csv](https://raw.githubusercontent.com/zief0002/epsy-8251/master/data/comic-characters.csv) data (see the [data codebook](http://zief0002.github.io/epsy-8251/codebooks/comic-characters.html)) to illustrate how to use several data wrangling ideas in order to answer a set of research questions about LGBTQ representation in comics. 
...
knitr::include_graphics("figs/20-click-data-object.png")
```
...
Use the `sort()` function to determine whether Marvel and DC introduced LGBTQ characters around the same time, or whether one company was more progressive?
:::
...
Rather than scrolling through the data, we could also have sorted the data so that the characters with LGBTQ status of "Yes" are printed first. To do this we want to sort the data initally (using the`lgbtq` column) in reverse alphabetical order ("Yes" followed by "No").
...
Use the `sort()` function to determine the first appearance of an LGBTQ female character in DC comics.
:::
...
There are many times in research applications that an educational scientist will need to select a subset of data cases. This type of application, for example, is quite common when we carry out demographic analyses (e.g., select the special education students, slect students on free/reduced-price lunch). To select a subset of rows from a tibble or data frame, we will pipe the data frame we want to select rows from into the `filter()` function. 
...
In both of the Mac and PC exmaples, the part of the pathname called `username` needs to be modified to be the user name for your computer. Typically this is the username you use to login to your computer.

:::exercises
**Your Turn**

Modify the syntax to export a CSV of the LGBTQ character data to the desktop on your computer. Also try modifying the syntax to export the CSV file to another folder/directory on your computer. 
:::
...
:::protip
If you are running this in the console, the `num_years` column won't be displayed because of the default printing options for tibbles; the new column is created, just not displayed. To view it, we can use the `print()` funtion with the argument `width=Inf`, which displays all columns in the tibble.
...
Modify the syntax to determine the average number of appearances for both DC and Marvel LGBTQ and non-LGBTQ characters (as well as the standard deviation and sample size) before and after the Pride Movement.  
:::
