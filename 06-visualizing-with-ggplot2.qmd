...
We will use the [gapminder.csv](https://raw.githubusercontent.com/zief0002/miniature-garbanzo/main/data/gapminder.csv) data (see the [data codebook](http://zief0002.github.io/miniature-garbanzo/codebooks/gapminder.html)) to illustrate how to create and modify visualizations using functionality in `{ggplot2}`.
...
# Read in data
gapminder = read_csv(file = "https://raw.githubusercontent.com/zief0002/miniature-garbanzo/main/data/gapminder.csv")
...
If you are creating visualizzations for a particular organization, there may be a palette of official colors associated with their brand. For example, the University of Minnesota's two official primary colors in hex notation (for electronic display) are:
...
```{r}
#| echo: false
knitr::include_graphics("figs/21-color-brewer-palette.png")
```
...
# Use the `{scales}` package to add $ in front of labels on the x-axis
ggplot(data = gapminder, aes(x = income, y = co2)) +  
  geom_smooth() +
  geom_point(shape = 21, size = 4, aes(fill = region)) +
  scale_fill_manual(
  ...
  scale_x_continuous(
    name = "Income (in thousands of international dollars, fixed for 2017 prices)",
    labels = dollar
  ) +
  theme_minimal()
```
...
Happy Plotting!!! `r emo::ji("graph")` `r emo::ji("party")`
